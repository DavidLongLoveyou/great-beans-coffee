...
test.describe('Product Catalog', () => {
  ...
  test('should load product catalog page', async ({ page }) => {
    ...
    // Check main heading
    await expect(
      page.getByRole('heading', { level: 1, name: /products|catalog|coffee/i })
    ).toBeVisible();
  ...
  test('should display product cards with essential information', async ({
    ...
    const originElement = firstCard.locator(
      '[data-testid="product-origin"], .origin'
    );
  ...
  test('should navigate to product detail page when clicking product card', async ({
    page,
  }) => {
    await page.waitForSelector('[data-testid="product-card"]');
    ...
    if (await originFilter.isVisible()) {
      ...
      if (await ethiopiaOption.isVisible()) {
        ...
        if (await firstCard.isVisible()) {
          ...
          const originText = await firstCard
            .locator('[data-testid="product-origin"]')
            .textContent();
    ...
    if (await processingFilter.isVisible()) {
      ...
      if (await washedOption.isVisible()) {
        ...
        if (await productCards.first().isVisible()) {
          const processingText = await productCards
            .first()
            .locator('[data-testid="product-processing"]')
            .textContent();
  ...
  test('should filter products by price range', async ({ page }) => {
    ...
    if (await priceFilter.isVisible()) {
      ...
      // Apply filter
      const applyButton = priceFilter.getByRole('button', {
        name: /apply|filter/i,
      });
  ...
  test('should search products by name', async ({ page }) => {
    ...
    if (await searchInput.isVisible()) {
      ...
      if (await productCards.first().isVisible()) {
        ...
        await expect(
          page.getByText(/no products found|no results/i)
        ).toBeVisible();
      }
    }
  });
  ...
  test('should sort products by different criteria', async ({ page }) => {
    ...
    if (await sortDropdown.isVisible()) {
      ...
      const priceLowToHigh = page.getByRole('option', {
        name: /price.*low.*high/i,
      });
  ...
  test('should handle pagination', async ({ page }) => {
    ...
    if (await pagination.isVisible()) {
      ...
      if ((await nextButton.isVisible()) && !(await nextButton.isDisabled())) {
        ...
        const prevButton = pagination.getByRole('button', {
  ...
  test('should display loading state while fetching products', async ({
    page,
  }) => {
  ...
  test('should handle empty search results', async ({ page }) => {
    ...
    if (await searchInput.isVisible()) {
      ...
      await expect(
        page.getByText(/no products found|no results|try different/i)
      ).toBeVisible();
      ...
      if (await suggestionsSection.isVisible()) {
        await expect(suggestionsSection).toContainText(
          /try|suggestion|browse/i
        );
      }
    }
  });
  ...
  test('should be responsive on mobile devices', async ({ page }) => {
    ...
    const mobileFiltersButton = page.getByRole('button', {
    ...
    if (await mobileFiltersButton.isVisible()) {
      ...
      // Close filters
      const closeButton = filtersModal.getByRole('button', {
        name: /close|×/i,
      });
  ...
  test('should add product to favorites/wishlist', async ({ page }) => {
    ...
    const firstProductCard = page
      .locator('[data-testid="product-card"]')
      .first();
    const favoriteButton = firstProductCard.getByRole('button', {
      name: /favorite|wishlist|heart/i,
    });
    ...
    if (await favoriteButton.isVisible()) {
      ...
      await expect(
        page.getByText(/added to favorites|added to wishlist/i)
      ).toBeVisible();
  ...
  test('should handle product comparison', async ({ page }) => {
    ...
    const firstCompareButton = productCards
      .first()
    ...
    if (await firstCompareButton.isVisible()) {
      ...
      const secondCompareButton = productCards
        .nth(1)
        .getByRole('button', { name: /compare/i });
      if (await secondCompareButton.isVisible()) {
        ...
        const viewComparisonButton = page.getByRole('button', {
          name: /view comparison|compare selected/i,
        });
  ...
  test('should track product view analytics', async ({ page }) => {
    ...
    const productViewEvent = analyticsEvents.find(
      event =>
        event.data?.includes('product_view') ||
        event.url.includes('product_view')
    );
