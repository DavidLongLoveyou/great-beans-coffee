...
test.describe('Contact Forms and Lead Generation', () => {
  test.describe('General Contact Form', () => {
    ...
    test('should validate required fields', async ({ page }) => {
      ...
      await expect(
        page.getByText(/email.*required|please enter.*email/i)
      ).toBeVisible();
      await expect(
    ...
    test('should submit contact form successfully', async ({ page }) => {
      ...
      await page
        .getByLabel(/message|inquiry/i)
        .fill(
          'Interested in your premium Arabica selection for our retail chain.'
        );
      ...
      await expect(
        page.getByText(/thank you|message sent|received/i)
      ).toBeVisible();

      // Should show confirmation details
      await expect(
        page.getByText(/contact.*soon|respond.*24.*hours/i)
      ).toBeVisible();
    });
    ...
    test('should handle form submission errors gracefully', async ({
      ...
      await expect(
        page.getByText(/error.*sending|try again|failed/i)
      ).toBeVisible();
  ...
  test.describe('Newsletter Subscription', () => {
    test('should display newsletter signup form', async ({ page }) => {
      ...
      await expect(
        page.getByLabel(/email.*newsletter|subscribe.*email/i)
      ).toBeVisible();
      await expect(
        page.getByRole('button', { name: /subscribe|sign up/i })
      ).toBeVisible();
    });
    ...
    test('should subscribe to newsletter successfully', async ({ page }) => {
      ...
      // Should show success message
      await expect(
        page.getByText(/subscribed|thank you.*subscribing/i)
      ).toBeVisible();
    ...
    test('should handle duplicate newsletter subscription', async ({
      page,
    }) => {
      ...
      await expect(
        page.getByText(/already subscribed|already registered/i)
      ).toBeVisible();
    });
  });

  test.describe('Quick Quote Request', () => {
    test('should display quick quote form on product pages', async ({
      page,
    }) => {
      ...
      await expect(
        page.getByRole('button', { name: /quick quote|get quote/i })
      ).toBeVisible();
    });
    ...
    test('should submit quick quote request', async ({ page }) => {
      ...
      const quickQuoteButton = page.getByRole('button', {
        name: /quick quote|get quote/i,
      });
      await quickQuoteButton.click();

      // Should show success message
      await expect(
        page.getByText(/quote request.*sent|contact.*soon/i)
      ).toBeVisible();
    });
  ...
  test.describe('Lead Magnet Downloads', () => {
    test('should display lead magnet forms', async ({ page }) => {
      ...
      if (await downloadForm.isVisible()) {
        ...
        await expect(page.getByLabel(/company/i)).toBeVisible();
        await expect(
          page.getByRole('button', { name: /download|get report/i })
        ).toBeVisible();
      }
    });
  ...
  test.describe('Multi-language Contact Forms', () => {
    ...
    test('should display validation messages in German', async ({ page }) => {
      ...
      const submitButton = page.getByRole('button', {
        name: /senden|abschicken/i,
      });
      await submitButton.click();

      // Should show German validation messages
      await expect(
        page.getByText(/name.*erforderlich|bitte.*name/i)
      ).toBeVisible();
      await expect(
        page.getByText(/e-mail.*erforderlich|bitte.*e-mail/i)
      ).toBeVisible();
    });
    ...
    test('should submit German contact form successfully', async ({ page }) => {
      ...
      await page
        .getByLabel(/nachricht|anfrage/i)
        .fill('Interesse an Ihren Premium-Arabica-Sorten für unsere Rösterei.');
      ...
      await expect(
        page.getByText(/vielen dank|nachricht.*gesendet|erhalten/i)
      ).toBeVisible();
    });
  });
  ...
  test.describe('Form Analytics and Tracking', () => {
    test('should track form interactions', async ({ page }) => {
      ...
      const formEvents = analyticsEvents.filter(
        e =>
          e.data?.includes('form_started') || e.data?.includes('form_submitted')
      );
