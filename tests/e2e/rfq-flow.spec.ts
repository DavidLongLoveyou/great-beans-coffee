...
test.describe('RFQ (Request for Quote) Flow', () => {
  ...
  test('should start RFQ from product detail page', async ({ page }) => {
    ...
    const requestQuoteButton = page.getByRole('button', {
      name: /request quote|get quote|inquire/i,
    });
  ...
  test('should validate quantity is positive number', async ({ page }) => {
    ...
    await expect(
      page.getByText(/quantity.*positive|quantity.*greater/i)
    ).toBeVisible();
  });
  ...
  test('should allow multiple product selection', async ({ page }) => {
    ...
    const addProductButton = page.getByRole('button', {
      name: /add product|select product/i,
    });
    if (await addProductButton.isVisible()) {
      ...
      if (await productModal.isVisible()) {
        const firstProduct = productModal
          .locator('[data-testid="product-option"]')
          .first();
        await firstProduct.click();

        // Add another product
        await addProductButton.click();
        const secondProduct = productModal
          .locator('[data-testid="product-option"]')
          .nth(1);
  ...
  test('should save draft and allow continuation', async ({ page }) => {
    ...
    const saveDraftButton = page.getByRole('button', {
      name: /save draft|save progress/i,
    });
  ...
  test('should handle file attachments', async ({ page }) => {
    ...
    if (await fileInput.isVisible()) {
      ...
      // Test file removal
      const removeFileButton = page.getByRole('button', {
        name: /remove.*file|delete.*file/i,
      });
      if (await removeFileButton.isVisible()) {
        await removeFileButton.click();
        await expect(
          page.getByText(/test-requirements\.pdf/i)
        ).not.toBeVisible();
      }
    }
  });
  ...
  test('should handle special requirements', async ({ page }) => {
    ...
    const specialRequirements = page.getByLabel(
      /special requirements|additional notes|comments/i
    );
    if (await specialRequirements.isVisible()) {
      const requirements =
        'Need organic certification and specific packaging requirements for retail distribution.';
  ...
  test('should show quote confirmation with reference number', async ({
    page,
  }) => {
    await page.goto('/quote');
    ...
    await expect(
      page.getByText(/quote.*submitted|request.*received/i)
    ).toBeVisible();
    ...
    await expect(
      page.getByText(/contact.*within|response.*hours|follow.*up/i)
    ).toBeVisible();
  });
  ...
  test('should send confirmation email', async ({ page }) => {
    ...
    await expect(
      page.getByText(/confirmation.*email|email.*sent/i)
    ).toBeVisible();
  });
  ...
  test('should handle network errors gracefully', async ({ page }) => {
    ...
    // Check error message
    await expect(
      page.getByText(/error.*submitting|network.*error|try.*again/i)
    ).toBeVisible();
  ...
  test('should track RFQ analytics events', async ({ page }) => {
    ...
    const rfqStartEvent = analyticsEvents.find(e =>
      e.data?.includes('rfq_started')
    );
    const rfqSubmitEvent = analyticsEvents.find(e =>
      e.data?.includes('rfq_submitted')
    );
