...
test.describe('PDF Generation', () => {
  ...
  test.describe('Product Specification PDF', () => {
    test('should generate PDF from product detail page', async ({ page }) => {
      ...
      const downloadButton = page.getByRole('button', {
        name: /download.*spec|download.*pdf/i,
      });
    ...
    test('should show loading state during PDF generation', async ({
      ...
      await expect(downloadButton).not.toContainText(/generating|loading/i, {
        timeout: 10000,
      });
    ...
    test('should handle PDF generation errors gracefully', async ({ page }) => {
      ...
      await expect(
        page.getByText(/error.*generating|failed.*download/i)
      ).toBeVisible();
    });
  });
  ...
  test.describe('RFQ Document PDF', () => {
    test('should generate PDF from RFQ detail page', async ({ page }) => {
      ...
      // Click the download PDF button
      const downloadButton = page.getByRole('button', {
        name: /download.*rfq|download.*quote/i,
      });
      await expect(downloadButton).toBeVisible();
      await downloadButton.click();
    ...
    test('should include all RFQ details in PDF', async ({ page }) => {
      ...
      const specialRequirements = page.getByLabel(
        /special requirements|additional notes/i
      );
      if (await specialRequirements.isVisible()) {
        await specialRequirements.fill(
          'Organic certification required. Custom packaging for retail distribution.'
        );
      }
