...
test.describe('Internationalization (i18n)', () => {
  ...
  test.describe('Language Navigation', () => {
    ...
    test('should switch between supported languages', async ({ page }) => {
      ...
      for (const locale of supportedLocales.slice(0, 4)) { // Test first 4 to avoid timeout
        const languageOption = page.getByRole('option', { name: new RegExp(locale, 'i') });
        await expect(languageOption).toBeVisible();
      }
      
      // Switch to German
      const germanOption = page.getByRole('option', { name: /deutsch|german|de/i });
    ...
    test('should maintain page context when switching languages', async ({ page }) => {
      ...
      // Switch to French
      const frenchOption = page.getByRole('option', { name: /français|french|fr/i });
  ...
  test.describe('Localized Content', () => {
    test('should display localized navigation menu', async ({ page }) => {
      ...
      await expect(page.getByRole('link', { name: /dienstleistungen/i })).toBeVisible();
    ...
    test('should display localized product information', async ({ page }) => {
      ...
      await expect(page.getByText(/origin|processing|tasting notes/i)).toBeVisible();
      
      // Switch to German product page
      await page.goto('/de/products');
      await page.waitForSelector('[data-testid="product-card"]');
      
      const germanProductCard = page.locator('[data-testid="product-card"]').first();
      await germanProductCard.click();
      
      // Should show German product details
      await expect(page.getByText(/herkunft|verarbeitung|geschmacksnoten/i)).toBeVisible();
    });
  ...
  test.describe('SEO and Meta Tags', () => {
    test('should have correct hreflang tags', async ({ page }) => {
      ...
      await expect(page.locator('link[hreflang="de"]')).toHaveAttribute('href', /\/de\//);
    ...
    test('should have localized meta tags', async ({ page }) => {
      ...
      await expect(englishDescription).toHaveAttribute('content', /coffee.*vietnam.*export/i);
      ...
      await expect(germanDescription).toHaveAttribute('content', /kaffee.*vietnam.*export/i);
    });

    test('should have correct Open Graph tags for different languages', async ({ page }) => {
      // English Open Graph
      await page.goto('/');
      await expect(page.locator('meta[property="og:locale"]')).toHaveAttribute('content', 'en_US');
      await expect(page.locator('meta[property="og:title"]')).toHaveAttribute('content', /The Great Beans/i);
  ...
  test.describe('URL Structure', () => {
    test('should have correct URL patterns for different locales', async ({ page }) => {
    ...
    test('should maintain deep links with locale switching', async ({ page }) => {
  ...
  test.describe('Date and Number Formatting', () => {
    test('should format dates according to locale', async ({ page }) => {
      ...
      if (await deliveryDate.isVisible()) {
        ...
        await expect(deliveryDate).toHaveAttribute('placeholder', /mm|dd|yyyy/i);
      }
      ...
      if (await germanDeliveryDate.isVisible()) {
        ...
        await expect(germanDeliveryDate).toHaveAttribute('placeholder', /dd|mm|yyyy/i);
      }
    });
