...
test.describe('Error Handling and Edge Cases', () => {
  test.describe('404 Error Pages', () => {
    ...
    test('should display 404 for non-existent product', async ({ page }) => {
      ...
      await expect(
        page.getByText(/product.*not found|product.*exist/i)
      ).toBeVisible();
      ...
      await expect(
        page.getByRole('link', { name: /browse.*products|view.*catalog/i })
      ).toBeVisible();
    });
    ...
    test('should display 404 for non-existent RFQ', async ({ page }) => {
      ...
      await expect(
        page.getByText(/rfq.*not found|quote.*not found/i)
      ).toBeVisible();

      // Should have link to create new RFQ
      await expect(
        page.getByRole('link', { name: /new.*quote|create.*rfq/i })
      ).toBeVisible();
    });
  ...
  test.describe('Network Error Handling', () => {
    test('should handle API timeouts gracefully', async ({ page }) => {
      ...
      await expect(
        page.getByText(/timeout|slow.*connection|try.*again/i)
    ...
    test('should handle server errors (500)', async ({ page }) => {
      ...
      await expect(
        page.getByText(/server error|something.*wrong|try.*later/i)
      ).toBeVisible();
      ...
      await expect(
        page.getByRole('button', { name: /retry|refresh/i })
      ).toBeVisible();
    });
    ...
    test('should handle network disconnection', async ({ page }) => {
      ...
      await expect(
        page.getByText(/offline|no.*connection|check.*connection/i)
      ).toBeVisible();
  ...
  test.describe('Form Error Handling', () => {
    test('should handle RFQ submission failures', async ({ page }) => {
      ...
      // Should show error message
      await expect(
        page.getByText(/error.*submitting|failed.*submit|try.*again/i)
      ).toBeVisible();
    ...
    test('should handle file upload failures', async ({ page }) => {
      ...
      if (await fileInput.isVisible()) {
        ...
        await expect(
          page.getByText(/file.*too.*large|size.*limit|reduce.*file/i)
        ).toBeVisible();
      }
    });
    ...
    test('should handle session expiration', async ({ page }) => {
      ...
      await expect(
        page.getByText(/session.*expired|please.*refresh|login.*again/i)
      ).toBeVisible();
  ...
  test.describe('Data Validation Edge Cases', () => {
    test('should handle extremely long input values', async ({ page }) => {
      ...
      await expect(
        page.getByText(/too long|character limit|maximum.*characters/i)
      ).toBeVisible();
    });
    ...
    test('should handle special characters in input', async ({ page }) => {
      ...
      await page
        .getByLabel(/name/i)
        .fill('José María Ñoño <script>alert("xss")</script>');
      await page.getByLabel(/email/i).fill('test+special@example.com');
      await page
        .getByLabel(/message/i)
        .fill('Testing with émojis 🚀 and symbols: @#$%^&*()');
