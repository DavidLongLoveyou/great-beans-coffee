...
test.describe('Homepage', () => {
  ...
  test('should navigate to products page from hero CTA', async ({ page }) => {
    const ctaButton = page.getByRole('button', {
      name: /explore products|view products|browse catalog/i,
    });
  ...
  test('should display featured products section', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: /featured|premium|popular/i })
    ).toBeVisible();
  ...
  test('should display market insights section', async ({ page }) => {
    ...
    if (await marketSection.isVisible()) {
      // Check section heading
      await expect(
        page.getByRole('heading', { name: /market|insights|trends/i })
      ).toBeVisible();

      // Check charts or data visualizations
      const charts = page.locator(
        '[data-testid="market-chart"], .chart, canvas'
      );
  ...
  test('should display company information section', async ({ page }) => {
    ...
    if (await aboutSection.isVisible()) {
      await expect(
        page.getByRole('heading', { name: /about|company|who we are/i })
      ).toBeVisible();
  ...
  test('should display testimonials section', async ({ page }) => {
    ...
    if (await testimonialsSection.isVisible()) {
      await expect(
        page.getByRole('heading', { name: /testimonials|reviews|clients/i })
      ).toBeVisible();
  ...
  test('should be responsive on mobile', async ({ page }) => {
    ...
    if (await mobileMenuButton.isVisible()) {
      ...
      await expect(
        mobileMenu.getByRole('link', { name: /products/i })
      ).toBeVisible();
    }
  ...
  test('should handle language switching', async ({ page }) => {
    ...
    if (await languageSwitcher.isVisible()) {
      ...
      const vietnameseOption = languageSwitcher.getByRole('option', {
        name: /vietnamese|tiếng việt|vi/i,
      });
      ...
      if (await vietnameseOption.isVisible()) {
        ...
        await expect(
          page.getByText(/cà phê|hạt cà phê|xuất khẩu/i)
        ).toBeVisible();
      }
    }
  });
  ...
  test('should track analytics events', async ({ page }) => {
    ...
    const ctaButton = page.getByRole('button', {
      name: /get quote|request quote/i,
    });
    if (await ctaButton.isVisible()) {
      await ctaButton.click();
    }
