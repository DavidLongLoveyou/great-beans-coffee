...
function performanceAudit() {
  ...
  if (fs.existsSync(nextConfigPath)) {
    ...
    const optimizations = [
      ...
      { feature: 'Bundle Analyzer', check: nextConfig.includes('bundleAnalyzer') },
      { feature: 'Compression', check: nextConfig.includes('compress') },
      { feature: 'Experimental Features', check: nextConfig.includes('experimental') },
      { feature: 'Webpack Optimization', check: nextConfig.includes('webpack') },
  ...
  if (imageStats.regularImages > 0) {
    console.log('⚠️ Consider replacing <img> tags with Next.js Image component for better performance');
  }
  ...
  if (totalJSSize > 5 * 1024 * 1024) { // > 5MB
    recommendations.push('🔴 JavaScript bundle is large (>5MB). Consider code splitting and lazy loading.');
  }
  
  if (jsFiles[0] && parseFloat(jsFiles[0].sizeMB) > 2) {
    recommendations.push('🔴 Largest JS chunk is >2MB. Consider breaking it down.');
  }
  
  if (dynamicImportsCount < 5) {
    recommendations.push('🟡 Low usage of dynamic imports. Consider lazy loading heavy components.');
  }
  
  if (imageStats.regularImages > imageStats.nextImages) {
    recommendations.push('🟡 More regular <img> tags than Next.js Image components. Optimize images.');
  }
