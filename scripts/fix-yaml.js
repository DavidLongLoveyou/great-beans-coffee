...
function fixYamlFile(filePath) {
  try {
    ...
    content = content.replace(/tableOfContents:\s*['"]?true['"]?\s*$/gm, 'tableOfContents: true');
    content = content.replace(/tableOfContents:\s*['"]?false['"]?\s*$/gm, 'tableOfContents: false');
    
    // Fix qualityLabs: ensure it's a proper number
    content = content.replace(/qualityLabs:\s*['"]?(\d+)['"]?\s*$/gm, 'qualityLabs: $1');
    
    // Fix equipmentLines: ensure it's a proper number
    content = content.replace(/equipmentLines:\s*['"]?(\d+)['"]?\s*$/gm, 'equipmentLines: $1');
