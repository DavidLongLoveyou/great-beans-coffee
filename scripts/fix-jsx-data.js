...
function fixJsxData(content) {
  ...
  const vietnameseRegionsPattern =
    /regions: \['{ name: 'Bắc Mỹ''[\s\S]*?'color: '#F4A460' }'\]/;
  if (vietnameseRegionsPattern.test(fixed)) {
    fixed = fixed.replace(
      vietnameseRegionsPattern,
      `regions: [
  ...
  const germanCountriesPattern =
    /countries: \['{ name: 'Deutschland''[\s\S]*?'marketValue: 2\.8 }'\]/;
  if (germanCountriesPattern.test(fixed)) {
    fixed = fixed.replace(
      germanCountriesPattern,
      `countries: [
...
const marketReportsDir = path.join(
  __dirname,
  '..',
  'content',
  'market-reports'
...
console.log(
  `Checking ${mdxFiles.length} market report files for JSX data issues...`
);
