...
function accurateSEOAudit() {
  ...
  function auditFile(filePath) {
    ...
    try {
      ...
      if (frontmatterBlocks.length > 1) {
        console.log(
          `❌ ${relativePath}: ${frontmatterBlocks.length} frontmatter blocks detected`
        );
      ...
      try {
        ...
        console.log(
          `❌ ${relativePath}: YAML parsing error - ${error.message}`
        );
        otherIssues++;
        return;
      }
  ...
  console.log(
    `  Files with duplicate frontmatter: ${duplicateFrontmatterFiles}`
  );
  console.log(`  Files with other SEO issues: ${otherIssues}`);
  console.log(
    `  Files with no issues: ${totalFiles - duplicateFrontmatterFiles - otherIssues}`
  );
}
