...
function fixSyntaxErrors(filePath) {
  try {
    ...
    if (content.includes('{ locale: _locale: _locale }')) {
      content = content.replace(/\{ locale: _locale: _locale \}/g, '{ locale: _locale }');
      modified = true;
    }
    ...
    const destructuringPattern = /const\s+\{[^}]+\}\s*=\s*await\s+params:\s*_params/g;
...
allFiles.forEach(file => {
  ...
  if (content.includes('params: _params') || content.includes('locale: _locale: _locale')) {
