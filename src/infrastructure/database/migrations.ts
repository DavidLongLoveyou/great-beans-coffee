...
export class DatabaseMigrations {
  ...
  private async validateForeignKeys(): Promise<void> {
    ...
    for (const check of fkChecks) {
      const result = await this.prisma.$queryRawUnsafe(check);
      // If any orphaned records found, validation fails
    }
  }
  ...
  private async validateUniqueConstraints(): Promise<void> {
    ...
    for (const check of uniqueChecks) {
      const result = await this.prisma.$queryRawUnsafe(check);
      // If any duplicates found, validation fails
    }
  }
  ...
  private async validateNotNullConstraints(): Promise<void> {
    ...
    for (const check of nullChecks) {
      const result = await this.prisma.$queryRawUnsafe(check);
      // If any null values found in required fields, validation fails
    }
  }
}
