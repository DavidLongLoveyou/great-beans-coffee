 ...
 */
export function isWithinBusinessHours(locale: Locale, date: Date = new Date()): boolean {
  const config = getMarketConfig(locale);
 ...
 */
export function getNextBusinessDay(locale: Locale, date: Date = new Date()): Date {
  const config = getMarketConfig(locale);
...
export function getCoffeeGradingStandards(locale: Locale): {
  ...
  const gradingStandards: Record<string, any> = {
    US: {
...
export function getPaymentTerms(locale: Locale): {
  ...
  const paymentTerms: Record<string, any> = {
    US: {
      preferredMethods: ['Letter of Credit', 'Wire Transfer', 'Documentary Collection'],
      standardTerms: ['30% advance, 70% on shipment', 'LC at sight', 'Net 30 days'],
      currency: config.currency.code,
    },
    DE: {
      preferredMethods: ['SEPA Transfer', 'Letter of Credit', 'Documentary Collection'],
      standardTerms: ['50% advance, 50% on delivery', 'LC 30 days', 'Net 45 days'],
      currency: config.currency.code,
    },
    JP: {
      preferredMethods: ['Letter of Credit', 'Bank Transfer', 'Documentary Collection'],
