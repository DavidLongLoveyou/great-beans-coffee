...
export function usePDFGeneration(): UsePDFGenerationReturn {
  ...
  const generateProductSpecSheet = useCallback(
    async (
      product: CoffeeProductEntity,
      options: PDFGenerationOptions = {}
    ) => {
      try {
        ...
        const pdfBlob = await pdfGenerationService.generateProductSpecSheet(
          product,
          {
            ...options,
  ...
  const generateRFQDocument = useCallback(
    async (rfq: RFQEntity, options: PDFGenerationOptions = {}) => {
      try {
        setIsGenerating(true);
        setError(null);
  ...
  const generateFromHTML = useCallback(
    async (
      ...
      try {
        ...
        const pdfBlob = await pdfGenerationService.generateFromHTML(
          element,
          filename,
          {
            ...options,
    ...
    async (
      ...
      try {
        ...
        toast({
          ...
          description: t('batch.generating.description', {
            count: items.length,
          }),
        });
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          try {
            ...
            if (item.type === 'product-spec-sheet' && 'sku' in item.data) {
              ...
              item.type === 'rfq-document' &&
              'rfqNumber' in item.data
            ) {
