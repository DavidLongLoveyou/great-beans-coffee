...
export type RFQStatus = 'pending' | 'processing' | 'quoted' | 'accepted' | 'rejected' | 'expired'
...
const allowedTransitions: StatusTransition[] = [
  ...
  { from: 'pending', to: 'rejected', label: 'Reject Request', requiresNote: true, confirmationMessage: 'Are you sure you want to reject this RFQ?' },
  { from: 'processing', to: 'quoted', label: 'Provide Quote' },
  { from: 'processing', to: 'rejected', label: 'Reject Request', requiresNote: true, confirmationMessage: 'Are you sure you want to reject this RFQ?' },
  { from: 'quoted', to: 'accepted', label: 'Mark as Accepted' },
  { from: 'quoted', to: 'rejected', label: 'Mark as Rejected', requiresNote: true },
    ...
    <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
      ...
      {availableTransitions.length === 1 ? (
        ...
        <Select onValueChange={(value) => {
          const transition = availableTransitions.find(t => `${t.from}-${t.to}` === value)
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={isLoading || (selectedTransition?.requiresNote && !note.trim())}
            >
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {history.map((entry, index) => {
          ...
          return (
            <div key={entry.id} className="flex items-start gap-3 p-3 border rounded-lg hover:shadow-sm transition-shadow">
              ...
              <div className="flex-1 space-y-1 min-w-0">
                ...
                {entry.note && (
                  <div className="text-xs bg-gray-50 p-2 rounded border-l-2 border-gray-200 sm:text-sm">
                    {entry.note}
                  </div>
                )}
              </div>
